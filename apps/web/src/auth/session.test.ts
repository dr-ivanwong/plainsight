...
function fakeTokenEndpoint(
  answer: Record<string, unknown> | 'refuse'
  ...
  const deps: AuthDeps = {
    fetchImpl: (async (url: unknown, init: unknown) => {
      ...
      if (answer === 'refuse') return new Response('{}', { status: 400 });
...
describe('the access token', () => {
  ...
  it('serves a live token without a network call', async () => {
    ...
    const { deps, calls } = fakeTokenEndpoint('refuse');
    await expect(getAccessToken(deps)).resolves.toBe('access-1');
    expect(calls).toHaveLength(0);
  });
  ...
  it('refreshes a dying token quietly', async () => {
    ...
    await expect(getAccessToken(deps)).resolves.toBe('access-2');
