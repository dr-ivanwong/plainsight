...
41 findings. After the 2026-07-19 pass (update note at the top): UI-1, INFRA-1 and FE-2 fixed, and four sync-convergence findings deferred under single-device operation. Open: 0 P0, 8 P1, 15 P2, 15 P3.
...
| X-1 | P1 | The sign-in → first-sync → upload path is unrehearsed; INFRA-1 and FE-2 now fixed, so BE-2 is the one live blocker left (and only on upload); second-device leg descoped |
...
| INFRA-4 | P2 | Dead `environment:*` OIDC trust subject survives the retired gate; any workflow environment on any ref can assume the deploy role |
...
### BE-2 · P1 · The upload job's "validating" stage does not validate
...
### BE-5 · P2 · The quota burns on failures and never refunds
...
### BE-6 · P2 · Pinned capabilities shipped narrower than spec, unamended
...
### FE-1 · P1 · No error boundaries; the pinned escape hatch does not exist
...
### FE-5 · P2 · Extraction can mint "not reported, treat as zero" without the user's assertion
...
### FE-8 · P2 · Two pinned CI stages do not exist
...
### UI-2 · P1 · The first screen states the superseded architecture as fact
...
### UI-3 · P1 · The pinned storage-pressure states were never built
...
### INFRA-2 · P1 · The PR diff job cannot assume the deploy role
...
### INFRA-3 · P1 · The cost-anomaly monitor likely watches a non-existent tag key, and the chain is unverified
...
### INFRA-4 · P2 · The retired gate left a live trust-policy widening
...
4. **Does a human checkpoint exist on stateful deploys?** ADR 0001 says yes (the gate, as a named compensating control); the amended cdk spec §7 says no (structural protections carry the load). Whichever way the owner settles it, one document must move, and branch protection on `main` is now the single load-bearing control either way; it cannot be verified from inside the repo.
...
6. **Export-pressure states under the new posture.** §12.9 demoted export from backup story to portability, but the spec still pins the quota banner and 30-day nudge and the build implemented neither (UI-3). Either they still matter (unsynced device-local writes are real) or the spec should say they no longer do; the current silence is neither.
