...
| INFRA-1 | P1 | Deployed CSP (`connect-src 'self'`) blocks the shipped sign-in token exchange; invariant test certifies the breaking value |
...
### FE-2 · P1 · A network blip at token refresh signs the device out
...
### INFRA-1 · P1 · The deployed CSP blocks the shipped sign-in flow
