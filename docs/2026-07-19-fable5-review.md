...
41 findings. After the 2026-07-19 pass (update note at the top): UI-1, INFRA-1, FE-2, BE-2, FE-1, FE-3, INFRA-2, INFRA-3, INFRA-4 and BE-5 fixed, and four sync-convergence findings deferred under single-device operation. Open: 0 P0, 3 P1, 13 P2, 15 P3.
...
5. **Make uploads honest before they are used (BE-2 done `4288746`; BE-5 done `9f68456`; FE-5, FE-6, BE-6 remain).** Gates in the worker (done: they run in the validating stage and the verdicts reach the reviewer) and quota refunds plus the runbook reset (done: the quota counts spend, not tries). What remains is user-only known-zero minting (FE-5), the atomic review save (FE-6), and the keep-source decision (BE-6).
