...
41 findings. After the 2026-07-19 pass (update note at the top): UI-1, INFRA-1, FE-2, BE-2, FE-1, FE-3, INFRA-2 and INFRA-3 fixed, and four sync-convergence findings deferred under single-device operation. Open: 0 P0, 3 P1, 15 P2, 15 P3.
...
6. **Close the pipeline's own gaps (INFRA-2 done `97d407b`; INFRA-3 done `36e4a4f`; INFRA-4 remains).** The read-only diff role landed, so the diff-is-the-review control can run once its variable is set, and the anomaly monitor now watches a tag key that exists with the chain-firing written up as a runbook drill. What remains is dropping the now-dead `environment:*` trust (INFRA-4).
