...
| Id | Section label | GICS source (the pre-fill mapping) |
|---|---|---|
| `healthcare` | Healthcare | Health Care |
| `technology` | Technology | Information Technology |
| `banks` | Banks | Financials: the Banks and Financial Services groups |
| `retail` | Retail | The two consumer Distribution & Retail groups |
| `resources` | Resources | Materials; Energy |
| `property` | Property | Real Estate |
| `industrials` | Industrials | Industrials |
| `insurance` | Insurance | Financials: the Insurance group |

*(Extended later the same day, owner-approved, for the coming ASX ingestion: `resources`, `property`, `industrials` and `insurance` join the owner's original four.)* The original four left the index's biggest blocks homeless; the gap plan's observation stands (financials and materials dominate the ASX), and BHP, the A-REITs and the insurers would all have piled into Unclassified. The third column pins the GICS mapping for the later import pre-fill slice: where a sector is named, all its industry groups map; the ASX listed-companies file carries the industry group per ticker, so the implementation maps at group grain. Any unmapped group (staples producers, telecom, utilities, the non-retail discretionary tail) arrives unclassified, deferred deliberately under the pinned rule: **a shelf earns its line when it has a tenant the owner intends to analyse; GICS having a box is not a reason.** Property joins banks in the dictionary caveat (main plan §12 entry 16): REITs strain the operating-company metric set, and a property-aware dictionary variant is not decided by this extension.

Pinned rules: vocabulary order is section order (the owner's original four, then additions in adoption order); absence is the unclassified state and is never encoded as a value (the §8 spirit: unknown is an absent key, not a sentinel); extension is a one-line amendment to this table, the same discipline as the §6 dictionary, no freestyle values anywhere; code writes the id (`healthcare`), and this table is the id-to-label mapping, per the naming decision (main plan §12 entry 8). **The sample mapping (judgement call one):** CSL and Cochlear file under `healthcare` (retiring "Medical devices"); Wesfarmers, Woolworths and JB Hi-Fi under `retail` (retiring "Conglomerate" and "Consumer staples"; Wesfarmers is the contestable call, filed by where its earnings live). **The free string retires (judgement call two):** everywhere the descriptive string rendered, the dashboard hero included, the section label now shows; a company whose one-word description carried nuance ("Medical devices") keeps that nuance in its thesis, not its label. Migration: a Dexie-versioned data pass rewrites known legacy strings per the mapping and clears unknown ones to absent for one-tap reassignment through the new details sheet (frontend §3), landing with the previous-schema fixture test §9 requires; the rewrite commits as an ordinary edit, so it queues, pushes, and converges the server copy (main plan §12.9). Export format unchanged: `formatVersion` stays 1 (§5, additive rule), and older export files normalise on import through the same mapping. The wire stays untouched in this slice: the api-contract company profile's `sector` remains provider-described free text (the ASX list's GICS industry group, EDGAR's SIC description; neither populated today), the sync payload stays opaque to the server, and the import pre-fill slice implements the table's GICS column when it lands (the EDGAR SIC side is drafted with that slice). **Judgement call three** lives in the frontend spec's details sheet: name and sector editable, ticker, exchange and currency fixed (a wrong identity or money field is a re-create, not an edit).

*Amendment (2026-07-22): the slice landed, and the shelves gained their first intended tenants.* The owner's capture request of this day, arriving alongside word of EOD price data for the ASX, named ten companies across five sector headings; the build took no market data anywhere, because the GICS industry group rides the listed-companies directory the search index already ingests, and manual price entry stands (main plan §12.1). The ask maps onto the vocabulary as pinned: Energy and Materials fold together into `resources` (owner-reaffirmed this day against a split shelf), the three banks file under `banks`, and the intended tenants stand recorded as this table's earn-its-line evidence: `resources` holds WDS, STO, BPT, BHP, RIO and FMG; `banks` holds CBA, NAB and ANZ; `healthcare` holds CSL and COH; `technology` holds WTC and XRO. Ticker hygiene from the same pass, verified against the directory as at 2026-07-22: the ask's WPL retired with Woodside's 2022 rename (it trades as WDS), APT delisted in 2022 after the Block acquisition (the owner chose XRO for the shelf), and CHM, which resolves to Chimeric Therapeutics, was the owner's slip for COH. Implementation landed as drafted above: the eight ids with the normalisation boundary in the client's record schema, the version-3 data pass with its fixture test, the grouped library, the add and details pickers, and the details sheet; the wire stayed untouched. The three judgement calls remain open for the owner's formal review, now inspectable in the running app.
