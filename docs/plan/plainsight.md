...
10. **Resolved: a desktop section rail inside a company (2026-07-18).** Prompted by the owner setting the app beside market-terminal UIs: at ≥1200px, company screens gain a left rail (a Library link, the company name, and Dashboard / Data entry / Thesis with the active section marked) so the high-frequency loop inside one company is a single click on desktop. What this evolves: the frontend spec §1.2 "no sidebar in v1" sentence, for company context only (§1.2 and §7 amended the same day). What stands: no global tab bar or sidebar at any width, the mobile stack untouched, and the rest of the market-terminal idiom considered and declined in the same conversation: charts keep the one-accent language with green, orange and red reserved for semantic health (§4), and live price charts stay out per §12.1's manual-price decision.
