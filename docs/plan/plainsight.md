...
**The binding constraint, stated by the product owner:** the app must remain fully functional independent of any AI service's availability. This drove the single most important architectural decision in this document: **the app is local-first**. (Superseded 2026-07-18: the backend is now the source of truth; see decision §12.9 for what changes and in what order. The AI-independence half of the sentence stands untouched.) All core analysis (ratio computation, trend charting, red-flag rules, comparison, thesis templates) runs entirely in the browser with data persisted locally. A backend exists only for optional enhancements (live market data, SEC filing ingestion, cross-device sync, AI-assisted narrative analysis), and every one of those enhancements degrades gracefully to the offline core.
...
2. Core analysis works offline against the last-synced copy: reads serve the synchronised working copy, writes queue with pending surfaced, and catch-up resumes on reconnect (the backend is the source of truth, decision §12.9). Independence from AI services is absolute: if every model provider on earth is down, the app still computes, charts, and stores. *(As built through Phase 3 the criterion was full zero-backend independence; superseded 2026-07-18.)*
...
1. **Not a brokerage or trading tool.** No order execution, no portfolio P&L tracking against live prices (v1). *(Amended 2026-07-22, §12 entry 17: the order-execution ban is absolute and survives the pairs sleeve, whose surfaces render and never trade; end-of-day P&L of the owner's own systematic book, computed by the engine and read from artefacts, is in scope and is not live-price tracking.)*
2. **Not investment advice.** The app computes and educates; it never says "buy" or "sell." Red flags are labelled as "items to investigate," not verdicts. A persistent disclaimer covers this. *(Scoped 2026-07-22, §12 entry 17: research and education surfaces keep the rule as written; the pairs sleeve's live surfaces describe the engine's own targets, positions and limits, the owner's own decisions, advising no one. The disclaimer extends to the sleeve.)*
...
Financial tools default to dense, intimidating, terminal-like interfaces. Plainsight keeps the density that serves reading the numbers and refuses the intimidation: an analyst's instrument panel built on **clarity, deference, depth** (direction amended 2026-07-19, §12 entry 12, staged by `2026-07-19-finance-look-gap.md`; the original wording answered the terminal aesthetic with "the opposite"). The numbers are the content; the interface recedes. The test for every screen: *does this feel inevitable, like it couldn't have been designed any other way?*
...
3. **Progressive disclosure.** The Learner sees a clean dashboard of 12 metrics, each carrying its sparkline and its recent years on the card face (loosened 2026-07-19, §12 entry 14: the budget counts metrics, not values, and five years of history moved on-screen with the key stats, the multi-year rows and the table view). Every deeper layer (formula, inputs, the full ten-year table, Owner's-lens essay) stays one tap away. The Practitioner can collapse the education layer globally.
...
> The five screens below define the design language by example. The complete inventory (all routes, the screens with their empty/loading/error states, and the component/hook contracts) is specified in the companion document **`plainsight-frontend.md`**.

**1. Library (home).** A calm list of saved companies. Each row: company name, ticker, a single composite quality indicator (small coloured dot: count of active red flags), last-updated date, and a 10-year ROE microsparkline. Generous 64px row height, no borders: separation by spacing alone. Pull-to-refresh triggers a sync when online; silently does nothing when offline. *(Amended 2026-07-19, §12 entry 16: rows group under quiet sector section headers from the companion's pinned vocabulary, unclassified last, the grouping invisible until a company is classified.)*
...
| PWA | **vite-plugin-pwa (Workbox)** | Installability + offline shell caching; the shell cold-starts offline and serves the catch-up mode (decision §12.9) |
...
*(Superseded 2026-07-18, decision §12.9: the backend is now the source of truth, and the bullets below read as the history of how the offline core was built. The current contract: DynamoDB holds the authoritative library; IndexedDB holds the synchronised working copy the app renders plus the queue of writes retried until the server accepts them; offline is a catch-up mode with pending state surfaced, never silently equal. The sync cadence is read-and-write driven rather than an overlay: reads revalidate on launch, reconnect, focus and sign-in; writes drain within seconds with exponential backoff while the server is unreachable; a signed-in device that has never synced holds placeholder rows on its first read rather than claiming an empty library. The service-worker shell, the sync wire itself, and export-as-portability stand unchanged.)*
...
**Degradation matrix (offline is catch-up mode, decision §12.9: reads serve the last-synced copy, writes queue and retry, pending is surfaced):**

| Feature | Online | Offline |
|---|---|---|
| Ratio dashboard, charts, red flags, compare, thesis | ✓ | ✓ against the last-synced working copy |
| Manual data entry | ✓ | ✓ writes queue as pending and drain on reconnect |
...
| Cross-device sync | ✓ | Queued locally with the pending count visible in Settings; catch-up on reconnect |
...
- **Upload mechanics:** presigned S3 PUT (annual reports run 5–30MB+, which rules out proxying through API Gateway's 10MB limit) → magic-byte and size validation (PDF/XLSX/CSV only, ≤50MB) → async extraction job (`POST /v1/extractions` returns a job id; client polls; 30–120s typical). Uploaded files are transient (lifecycle-deleted after 7 days). *(Amended 2026-07-20, with backend spec §6: shipped PDF-first, the XLSX/CSV stage staying pinned as roadmap; and the "keep source document" toggle is removed rather than deferred, by owner decision. Source checking happens during review from the job's retained bytes, and after a save the provenance's file name and page number name the reviewer's own copy of a re-downloadable public filing, so a durable server-side copy was declined at one seat.)*
...
**BYOK: the user's own keys, entered in Settings.** The registry supports a second credential source beyond the server's SSM SecureString store: keys the user pastes into a Settings → Providers screen. For a single-user app whose keys never leave the device this is the natural pattern, and it changes the architecture for the better:
...
- **The client rides out an outage; the backend owns the data (decision §12.9).** Total backend failure degrades the product to the working app on its last-synced copy: reads serve the cache, writes queue with pending surfaced, and catch-up resumes when the backend returns. The price of holding the authoritative library is that the backend is a standing obligation, kept deployed, reachable, and restorable, with PITR as the recovery story rather than belt-and-braces. *(The original framing, total backend failure costing nothing because the client was local-first, reads as the history of Phases 1 and 2.)*
...
**Phase 3 (auth + sync):** Cognito (hosted UI, no password handling by us) + sync Lambdas + a `SYNC#{userId}` item collection in the same DynamoDB table. DynamoDB PITR turned on the day user data lands (RPO ≈ 5 min; RTO: hours, acceptable at one user). Since decision §12.9 this table holds the authoritative library, so PITR is the recovery story; each device's synchronised working copy and the export file are the secondary copies that keep the owner reading and writing while a restore runs.
...
| **1: Offline core (MVP)** | PWA shell, Library, data entry, dashboard, metric details, red-flag engine, export/import, dark mode, S3+CF deploy | 3–4 wks | Full Journey A completable in airplane mode (criterion as met at the time; the airplane-mode property reads as history since decision §12.9); Lighthouse PWA ✓; budgets met |
...
| **3: Compare + Thesis + Sync + Uploads** | Comparison view, thesis editor with versioned snapshots, Cognito auth, multi-device sync, BYOK provider settings screen, filing upload + extraction review mode (client-direct BYOK by default; auth'd server-proxy pass-through for non-CORS providers) | 3–4 wks | Two devices converge after offline edits on both (descoped to single-device operation 2026-07-19, §12.15; the convergence proof stays in tests); thesis history immutable; uploaded annual report → reviewed and saved company in < 5 min; export file verified key-free |
...
| IndexedDB eviction / storage loss on device | Low | Med | `navigator.storage.persist()` requested; prominent export habit built into UX (periodic "back up your library" nudge); since decision §12.9 the authoritative copy is server-side, so eviction costs at most the unsynced queue and a re-pull |
| Regulatory perception: "is this investment advice?" | Low | High | Non-goal #2 enforced in copy review: no buy/sell language on research and education surfaces (scoped for the pairs sleeve 2026-07-22, §12 entry 17: its live surfaces describe the engine's own positions, advising no one), education framing, persistent disclaimer; red flags phrased as questions to investigate |
...
9. **Resolved: the backend becomes the source of truth (2026-07-18; supersedes the local-first constraint).** Owner's call at the end of the Phase 3 build: the authoritative copy of the library lives in DynamoDB; the client holds a synchronised working copy and retries every write until the server accepts it. What this supersedes: the §1 binding-constraint paragraph, the §5 local-first data layer ("IndexedDB is the source of truth"), the degradation-matrix promise that every feature works with zero backend dependency, the Phase 1 airplane-mode exit criterion, and the "deepest reliability property" framing in §6; those passages now read as the history of how the offline core was built, not as the operating contract. Consequences accepted with eyes open: the backend becomes a standing obligation (kept deployed, reachable, and restorable; PITR moves from belt-and-braces to the recovery story); the owner's research is warehoused server-side, so the privacy posture rests on Cognito and encryption at rest rather than on data never leaving the device; sign-in stops being optional for durable use; and offline becomes a catch-up mode (reads serve the last-synced copy, writes queue and retry) rather than full independent function. Migration lands in slices: first the reconciler flips to server-wins (a pulled server copy beats a dirty local edit; pending local writes surface as pending instead of silently equal), then reads move behind the API with IndexedDB demoted to cache-and-queue, then the documentation pass rewrites the degradation matrix, success criteria, and DR posture to match. Export remains as data portability, no longer as the backup story. Rollback trigger: if the standing obligation stops being worth it for a single user, this entry reverses cleanly while the sync protocol (backend spec §4) stays the same wire either way. All three slices landed 2026-07-18: the server-wins reconciler with the pending surface, the read-and-write-driven sync cadence with the first-catch-up gate, and this documentation pass.
10. **Resolved: a desktop section rail inside a company (2026-07-18).** Prompted by the owner setting the app beside market-terminal UIs: at ≥1200px, company screens gain a left rail (a Library link, the company name, and Dashboard / Data entry / Thesis with the active section marked) so the high-frequency loop inside one company is a single click on desktop. What this evolves: the frontend spec §1.2 "no sidebar in v1" sentence, for company context only (§1.2 and §7 amended the same day). What stands: no global tab bar or sidebar at any width (the desktop half of this clause superseded the same day, entry 11; the mobile refusal stands), the mobile stack untouched, and the rest of the market-terminal idiom considered and declined in the same conversation: charts keep the one-accent language with green, orange and red reserved for semantic health (§4), and live price charts stay out per §12.1's manual-price decision.
11. **Resolved: the rail becomes the app's desktop navigation (2026-07-18, later the same day as entry 10).** Owner's call after seeing the company rail live: at ≥1200px the rail is persistent on every screen except the welcome flow, carrying the top-level destinations (Library; Compare once two companies exist, the library's progressive rule in the frontend spec §3; Settings) and, inside a company, the company's name and sections. This supersedes entry 10's no-global-sidebar clause for desktop widths only; the mobile tab-bar refusal and the sub-1200 stack are untouched, and per-screen top-bar navigation duplicates recede at the breakpoint while titles, actions, and the autosave status line stay. Entry 10's declined half is undisturbed: no red or green price idiom, no live quotes.
12. **Resolved: the design language aims at the instrument panel (2026-07-19).** The owner set the app beside broker dashboards and market pages and asked for the finance look. What that means, separated in `2026-07-19-finance-look-gap.md`: the aesthetic of a professional fundamentals tool (direction as colour, history in figures, charts beside numbers, a dense tabular mode) arrives by finishing the dashboard design plan of 2026-07-13 and extending its language to the library; the content of a market terminal (live quotes, news, movers) stays out per entries 1, 10 and 11 of this log. §4's philosophy paragraph was reworded the same day from "we apply the opposite" to the instrument-panel reading. First code under the new direction: the dashboard's trends section (small multiples with a table fallback, dashboard design plan §6). The remaining stages sit behind the owner confirmations the gap plan lists (delta colour with direction pinning, the disclosure loosening, benchmark defaults) and land as those arrive.
13. **Resolved: trend direction wears semantic colour (2026-07-19; supersedes the neutral-chip stance).** Stage 2 of the finance-look gap plan, owner-approved. The delta chip colours by the metric's own-trend health direction, now pinned in the dictionary (data-model §6 note N6, the new optional `healthDirection`): green when the five-year move runs with it, orange against it, neutral grey for a flat move or an unpinned direction. Current ratio is pinned direction-neutral (an ever-fatter one is not obviously healthier) and the valuation metrics carry no direction (their moves describe the price paid); both stay grey everywhere. A card-level health dot and the sparkline's colour carry the same computed signal, with a fired, undismissed rule outranking the trend through the rule-to-card map the dashboard design plan drafted (§4.2, adopted as drafted; dilution names no card). The compare screen's best-in-row tick is deliberately untouched: it ranks peers at a point in time through the separate comparison preference, so the group's lowest P/E keeps its tick while no P/E trend ever wears colour. Two text-grade palette steps (`healthyText`, `investigateText`) were minted so 13px chip figures hold 4.5:1 where the base semantic pair is held to 3:1 as graphics; the squint test (§4) now works as written.
14. **Resolved: history moves on-screen (2026-07-19; loosens the progressive-disclosure wording).** Stage 3 of the finance-look gap plan, owner-approved: "a dashboard of 12 numbers, one tap away" becomes a dashboard of 12 metrics with their recent history visible. What landed together: the multi-year row on every card face (the latest five labelled years in 11px tabular figures, short degenerate forms speaking their full phrases), and the key-stats header (ROE, net margin, debt-to-equity and free cash flow at 22px between the hero and the grid, each opening its detail sheet). What does not move: the metric budget stays twelve cards; the detail sheet remains the home of the full run of years, the formula and the inputs; nothing new is computed, only already-held figures made visible. §4's third governing decision was reworded the same day; CLAUDE.md's product-discipline line mirrors it; the dashboard design plan §4.6 and §5.3 carry the build notes.
15. **Resolved: single-device operation, backend still the source of truth (2026-07-19; descopes the two-device Phase 3 exit criterion).** Owner's call after the four-lens review (`docs/2026-07-19-fable5-review.md`): the app is run from a single device, and a second is not being converged. What stands untouched: the backend as the source of truth (§12.9), the single-seat Cognito pool, and sign-in as the price of durable use; the authoritative library and the sign-in path do not change. What this supersedes: the Phase 3 exit criterion in §8, "Two devices converge after offline edits on both," now descoped, and any reading of the runbook or the risk table that treats a second real device as imminent. What does not change in the code: the sync protocol (backend spec §4) stays built, correct by design, and tested, two-device convergence proof included; this decision retires a go-live gate, not a capability. Consequence for the review: its sync-convergence findings, the ones that need a concurrent second writer to bite (the four deferred there: the pull-cursor skip, the tombstone-marks race, resync-reconciles-absence, and the wire-parse wedge), drop to latent, each with a pinned reactivation trigger; the sign-in and upload findings are untouched, because one device still signs in and still uploads. Reversal trigger, stated so it is not crossed by accident: the day a second device is added, the deferred convergence work becomes the pre-condition (a DynamoDB Local integration pass over push, pull, tombstone and resync, then a rehearsed two-device walk), and this entry's descoping lifts. This narrows the single-operator direction of the hedge-fund gap draft (2026-07-18), which had assumed the owner's own multiple devices; that draft stays outside the authority set until pinned.

16. **Resolved: the library groups companies by a pinned sector vocabulary; ETFs are the fund shelf, not a sector (2026-07-19).** Prompted by the owner's ask of the same day for library sections: healthcare, technology, banks, retail, and ETFs. The ask splits three ways on arrival. **Sectors:** the company record's free-text `sector` becomes an id from the vocabulary pinned in the companion (§12 D3: `healthcare`, `technology`, `banks`, `retail`; extension is a one-line amendment there), because free text fragments into one section per company (the sample five carry five different strings, Cochlear filed outside Healthcare); the library groups under quiet section headers in vocabulary order, unclassified last and invisible until anything is classified, and the dashboard hero gains the company details sheet, the first edit surface for name and sector (frontend §1.1, §3). **ETFs:** not a sector value. A fund is a different entity, scoped by the fund-shelf draft of 2026-07-19, which stays outside the authority set until its own decision-log entry adopts it; nothing here pre-commits it. **Banks:** group today as ordinary companies while the metric dictionary keeps its operating-company shape; several cards go honestly not-meaningful on a bank (no cost of goods behind gross margin, deposit-funded leverage, free-cash-flow semantics that mislead), and a financials-aware dictionary variant is explicitly not decided by this entry; if bank analysis becomes central, it arrives as its own scoped decision with the fund shelf's discipline. Deliberately out of the slice: import pre-fill (mapping the ASX list's GICS industry group or EDGAR's SIC description to a suggested value) waits for a later slice, and the wire and server change not at all. Three drafter judgement calls await the owner, recorded in the companion's D3: the sample mapping (Wesfarmers and Woolworths under Retail, Cochlear under Healthcare), the retirement of the descriptive free string from the hero, and the details sheet keeping ticker, exchange and currency fixed. *Amended later the same day, owner-approved: the vocabulary extends from four shelves to eight for the coming ASX ingestion (`resources`, `property`, `industrials`, `insurance`); the companion's D3 table gains the pinned GICS mapping column that makes the import pre-fill slice near-free, plus the earn-its-line rule for anything further (a shelf earns its line when it has a tenant the owner intends to analyse). Property joins banks in the dictionary caveat: REITs strain the operating-company metric set (fair-value gains through earnings, development capex inside free cash flow, structural gearing), and a property-aware dictionary variant is likewise not decided here.* *Amended 2026-07-22: the slice landed, approved by the owner through the sector-capture request of the same day; the companion's D3 records the build and the first intended tenants per shelf, with the ticker corrections that request surfaced. The ask arrived alongside word of EOD price data for the ASX, and none entered the build: classification rides the free listed-companies directory's GICS column, entry 1's manual-price decision stands untouched, and any future price feed remains its own scoped decision (the hedge fund gap plan maps it), not a rider on a grouping slice. The three judgement calls stay open for formal review, now inspectable in the running app.*

17. **Resolved: the pairs sleeve is chartered (2026-07-22).** Owner's call, proceeding on the pairs research integration proposal of the same day (`2026-07-22-pairs-research-integration.md`, pinned by this entry as the sleeve's build contract and joining the authority set): Plainsight expands to hold a second discipline, a pairs-trading research and monitoring area, so the operator who learns to read statements here also researches, validates and monitors the pairs programme here. The shape in three sentences: a Python engine (`quant/pairs-engine`, recorded as ADR 0005) runs the statistics and the broker loop on the operator's machine and writes versioned artefacts; the app gains one top-level rail item, **Pairs** (research, backtest, live: the drafted answer to the contract's naming question, standing unless the owner renames it), rendering those artefacts with the house audit posture; and the app never trades and never writes sleeve data, so order placement, the local book and the reconcile-before-trade halt stay with the engine and the broker. Statistics select pairs, fundamentals qualify them: the fundamentals engine emits no signals, the pairs engine never touches the metric dictionary, and the join is the research surface showing each candidate's legs against their sector, red flags and dashboards. Amended in place this same day: non-goal 1 (execution banned absolutely; end-of-day P&L of the owner's own book renders from artefacts) and non-goal 2 (the buy/sell-language rule scoped: the sleeve's live surfaces describe the engine's own targets and positions, advising no one). Standing untouched, deliberately: entry 1 (the sleeve's closes live engine-side behind its own data module, the first tranche of the adapter interface that entry reserved; no company price record changes meaning), entry 5 (one seat), entries 10 to 12 (the rail and the instrument-panel direction, which the sleeve serves with dense end-of-day figures and no quotes, news or movers), non-goal 4 (close-to-close by construction), the 12-card budget (surfaces, not cards), entry 9 (artefacts are server-truth; the client renders a synchronised read-only copy), and the constraint that nothing in the client serving path calls a model (the engine is deterministic statistics, offline from the app's point of view). The strategy itself (universe, capital, stops, weekly criteria, the go/no-go gate) stays governed by the separately operated pairs trading plan of 2026-07-22; this entry charters what is built in this repository, staged as the contract's six slices, of which this record is slice 0. Open at charter time, written down so it is not crossed silently: halt alerting ahead of live capital (contract §10). The hedge fund gap plan remains outside the authority set, sharing the data-licence direction and nothing else of its scope.

18. **Resolved: the first-run welcome is removed (2026-07-22).** Owner's call: the three-pane welcome (frontend spec S1) put a stop between launch and the library without saying anything the app does not already say in place. A true first launch now lands on the library's true-empty state, whose hero carries the same promise and the same two starts, Add a company and See it with sample data; the welcome's third start, import a file, stays reachable through the library's import affordances and the entry screen's upload sheet. What leaves with it: the welcome route and screen, the root redirect and the flag gate behind it, the rail's welcome-flow exception (entries 10 and 11 as amended: at ≥1200px the rail now sits on every screen), and the settings About row that replayed it. What stands: the data-residency copy the welcome's second pane carried is stated and pinned by test in the settings sync group (entry 9), and the sample-data decision (frontend spec §4) is untouched. The stored first-launch flag itself retires in a follow-up slice with a migration that deletes the legacy row, so a live device's row is removed rather than quarantined; until then the key remains in the schema, written by no one. *(The retirement landed 2026-07-23: schema version 5 deletes the legacy row on upgrade, and the key left the meta record union and the export allowlist. Older export files still import; the retired key strips at parse as an unknown field, and the format version holds at 1 under the additive rule.)*
...
The caveat that originally threatened the local-first premise: **WebKit's Intelligent Tracking Prevention deletes all script-writable storage, including IndexedDB, after 7 days without interaction, for web apps not installed to the home screen.** Installed PWAs are exempt. Since decision §12.9 the stakes are lower: eviction costs the device its working copy and any unsynced queue, and the library re-pulls from the server on the next signed-in launch; the mitigations remain because re-downloading is friction and unsynced edits are real. In order: (1) on iOS Safari, a one-time, dismissible, honestly-worded "Add to Home Screen" explainer on the Library screen; (2) `navigator.storage.persist()` requested wherever honoured; (3) a storage-status readout in Settings → Data (persisted?, usage vs quota via `storage.estimate()`); (4) an export nudge when no backup has been taken in 30 days; (5) the server-side authoritative copy, which turns eviction from data loss into a re-sync. Quota exhaustion is detected proactively and surfaced with an export prompt before writes begin failing.
...
*(Epigraph annotated 2026-07-20: decision §12.9 made the backend the source of truth, so the durable library now does depend on our own backend; the sentence stands as the founding posture's record. What survives untouched is the reading promise itself: analysis runs in the browser, offline, against the synchronised copy, and no AI service is ever load-bearing.)*
