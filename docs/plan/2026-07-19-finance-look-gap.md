...
3. **Rule-to-metric map (gates stage 2).** The draft in the dashboard design plan §4.2. Recommendation: adopt as drafted.
4. **Progressive-disclosure loosening (gates stage 3).** Recommendation: yes; without it the dashboard cannot show a year of history without a tap, and that tap is the distance between reading app and instrument.
