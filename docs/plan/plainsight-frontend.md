...
**One-tap sample load, not silent preload.** The empty Library offers "See it with sample data"; tapping it loads **CSL**, the ASX golden-corpus showcase, as `sample: true` records (schema flag per companion §9), badged and removable with one action in S11. *(Amended 2026-07-18 with the ASX-first steer: the original US trio, Apple, Coca-Cola, and Costco, retired from the sample set per companion §12 D1's amendment. CSL alone remains because the companion's depth rationale wants ten-year fixtures behind sample sparklines and CSL is the ASX fixture that carries them; further ASX samples join when a second fixture is deepened to ten years or the depth rule is amended. Until then a sample library of one cannot reach Compare, which is accepted.)*
