...
describe('earnings quality', () => {
  ...
  it('fires when OCF trails NI for 3 years and cumulative coverage is under 0.9', () => {
    const flag = fired(run(above, below, below, below), 'earningsQuality');
    expect(flag).toMatchObject({
      ruleId: 'earningsQuality',
      severity: 'orange',
      window: ['FY2021', 'FY2022', 'FY2023']
    });
  ...
  it('does not fire when cumulative coverage reaches 0.9', () => {
    ...
    expect(fired(run(wobble, wobble, wobble), 'earningsQuality')).toBeUndefined();
  });

  it('does not fire when any of the three years covers net income', () => {
    expect(fired(run(below, above, below), 'earningsQuality')).toBeUndefined();
  });
  ...
  it('abstains when the window has a gap', () => {
    ...
    expect(fired(years, 'earningsQuality')).toBeUndefined();
  });
  ...
  it('stays silent when cumulative net income is not positive (interpretation note)', () => {
    ...
    expect(fired(run(losses, losses, losses), 'earningsQuality')).toBeUndefined();
  });
});

describe('eroding moat', () => {
  ...
  it('fires orange at 3 declining steps with a cumulative fall of at least 2 pp', () => {
    const flag = fired(run(gm(46), gm(44), gm(42), gm(40)), 'erodingMoat');
    expect(flag).toMatchObject({ ruleId: 'erodingMoat', severity: 'orange' });
  ...
  it('goes red at 5 or more declining steps', () => {
    const flag = fired(run(gm(46), gm(44), gm(42), gm(40), gm(38), gm(36)), 'erodingMoat');
  ...
  it('does not fire when the cumulative decline stays under 2 pp (basis-point drift)', () => {
    expect(fired(run(gm(46), gm(45.5), gm(45.1), gm(44.8)), 'erodingMoat')).toBeUndefined();
  });

  it('does not fire at only 2 declining steps', () => {
    expect(fired(run(gm(46), gm(44), gm(42)), 'erodingMoat')).toBeUndefined();
  });

  it('a gap year breaks the streak (data sufficiency)', () => {
    const years = [
    ...
    expect(fired(years, 'erodingMoat')).toBeUndefined();
  });
  ...
  it('reports both margins when both erode', () => {
    ...
    const flag = fired(run(both(46), both(44), both(42), both(40)), 'erodingMoat');
    expect(flag?.firedWith.margin).toBe('gross and operating');
  });
});

describe('leverage-flattered returns', () => {
  // Baseline debt-to-equity = 0.5 (20k debt / 40k equity), ROE = 0.5.
  ...
  it('fires when D/E rises 0.3 while ROE moves no more than 1 pp', () => {
    ...
    const flag = fired(run({}, {}, {}, leveredUp), 'leverageFlatteredReturns');
    expect(flag).toMatchObject({ ruleId: 'leverageFlatteredReturns', severity: 'orange' });
    expect(flag?.window).toEqual(['FY2020', 'FY2021', 'FY2022', 'FY2023']);
  });
  ...
  it('does not fire when the D/E rise is under 0.3', () => {
    ...
    expect(fired(run({}, {}, {}, mild), 'leverageFlatteredReturns')).toBeUndefined();
  });

  it('does not fire when ROE rose more than 1 pp alongside', () => {
    const earning: ValueSpec = { ...leveredUp, netIncome: 24_000 };
    expect(fired(run({}, {}, {}, earning), 'leverageFlatteredReturns')).toBeUndefined();
  });

  it('abstains without four consecutive labels', () => {
    expect(fired(run({}, {}, leveredUp), 'leverageFlatteredReturns')).toBeUndefined();
  });
  ...
  it('abstains when an endpoint metric does not compute', () => {
    ...
    expect(fired(run(negativeEquityStart, {}, {}, leveredUp), 'leverageFlatteredReturns')).toBeUndefined();
  });
});
...
describe('fragility', () => {
  it('fires orange below 3x coverage', () => {
    ...
    const flag = fired(run({ interestExpense: 10_000 }), 'fragility');
    expect(flag).toMatchObject({ ruleId: 'fragility', severity: 'orange', window: ['FY2020'] });
    expect(flag?.firedWith.coverageDisplay).toBe('2.5×');
  });

  it('fires red below 1.5x and when coverage is negative', () => {
    expect(fired(run({ interestExpense: 20_000 }), 'fragility')).toMatchObject({ severity: 'red' });
    const negative = fired(run({ operatingIncome: -5_000, interestExpense: 10_000 }), 'fragility');
  ...
  it('does not fire at 3x or better', () => {
    expect(fired(run({ interestExpense: 8_000 }), 'fragility')).toBeUndefined(); // 3.125x
  });

  it('abstains for the no-debt state', () => {
    expect(fired(run({ interestExpense: zeroAsserted }), 'fragility')).toBeUndefined();
    expect(fired(run({ interestExpense: 0 }), 'fragility')).toBeUndefined();
  });

  it('abstains when the latest year cannot compute coverage', () => {
    const years = [completeYear('FY2020', {}, { drop: ['operatingIncome'] })];
    expect(fired(years, 'fragility')).toBeUndefined();
  });
});
...
describe('dilution', () => {
  ...
  it('fires when share CAGR tops 2%/yr without commensurate revenue growth', () => {
    const flag = fired(run(sh(1_000), sh(1_060), sh(1_124), sh(1_191)), 'dilution');
    expect(flag).toMatchObject({ ruleId: 'dilution', severity: 'orange' });
    expect(flag?.window).toEqual(['FY2020', 'FY2021', 'FY2022', 'FY2023']);
  });
  ...
  it('does not fire when revenue grows at least twice as fast as the share count', () => {
    ...
    expect(fired(years, 'dilution')).toBeUndefined();
  });

  it('does not fire at 2%/yr share growth or less', () => {
    expect(fired(run(sh(1_000), sh(1_015), sh(1_030), sh(1_045)), 'dilution')).toBeUndefined();
  });

  it('abstains on degenerate endpoints', () => {
    expect(fired(run(sh(0), sh(1_060), sh(1_124), sh(1_191)), 'dilution')).toBeUndefined();
    expect(fired(run({ ...sh(1_000), revenue: 0 }, sh(1_060), sh(1_124), sh(1_191)), 'dilution')).toBeUndefined();
  });

  it('abstains when the window is not covered', () => {
    expect(fired(run(sh(1_000), sh(1_060), sh(1_124)), 'dilution')).toBeUndefined();
  });
...
describe('manufactured returns', () => {
  ...
  it('fires when latest ROE tops 25% on debt-to-equity above 2', () => {
    const flag = fired(run(shrunkEquity), 'manufacturedReturns');
    expect(flag).toMatchObject({ ruleId: 'manufacturedReturns', severity: 'orange', window: ['FY2020'] });
...
describe('capital-intensity creep', () => {
  it('fires when revenue rises and FCF falls in each of the latest two steps', () => {
    ...
    const flag = fired(years, 'capitalIntensityCreep');
    expect(flag).toMatchObject({ ruleId: 'capitalIntensityCreep', severity: 'orange' });
  ...
  it('does not fire when either step breaks the pattern', () => {
    ...
    expect(fired(revenueDips, 'capitalIntensityCreep')).toBeUndefined();
    const fcfRecovers = run(
      { revenue: 100_000, operatingCashFlow: 22_000 },
      { revenue: 110_000, operatingCashFlow: 20_000 },
      { revenue: 120_000, operatingCashFlow: 21_000 }
    );
    expect(fired(fcfRecovers, 'capitalIntensityCreep')).toBeUndefined();
  });
  ...
  it('abstains on a gap or a missing input', () => {
    ...
    expect(fired(gap, 'capitalIntensityCreep')).toBeUndefined();
    const missing = [
      completeYear('FY2020'),
      completeYear('FY2021'),
      completeYear('FY2022', {}, { drop: ['capex'] })
    ];
    expect(fired(missing, 'capitalIntensityCreep')).toBeUndefined();
  });
});
