...
**The backend is the source of truth (decision §12.9 of the main plan, 2026-07-18, superseding the original local-first constraint).** The authoritative library lives in DynamoDB; the client holds a synchronised working copy in IndexedDB and retries every write until the server accepts it. Offline is a catch-up mode: reads serve the last-synced copy, writes queue and retry, and pending state is surfaced, never silently equal. The migration is landing in slices: server-wins reconciliation landed 2026-07-18 (a pulled server copy beats a dirty local edit, and the settings sync row counts the writes still waiting); reads behind the API landed the same day (the working copy revalidates on launch, reconnect, focus and sign-in; queued writes drain within seconds and retry with backoff until accepted; a signed-in device that has never synced holds placeholder rows rather than claiming an empty library). The documentation pass remains; older passages in the plans describe the local-first history until it rewrites them. What survives unchanged: nothing in the client serving path may ever call a model, and AI features must degrade to a working app.
