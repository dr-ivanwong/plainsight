...
**Current state: Phase 2 (import) code-complete 2026-07-12; the deployed exit pass awaits the owner's go-live steps ([docs/runbook.md](docs/runbook.md)).** The workspace: `packages/calc-engine` (complete per the data-model spec, 100% branch coverage enforced, golden corpus green), `packages/api-contract` (the wire contract as Zod schemas, shared by Lambdas and the client), `apps/api` (the EDGAR mapping held to the golden corpus by integer equality, plus the read/search/ingest/sweep/kill-switch handlers), `apps/web` (the offline core: Dexie layer, PWA shell, onboarding, library, dashboard, entry, settings, Journey A green in airplane mode; plus Journey B's import sheet with the ingesting retry state), `infra/` (six stacks, `features.api`/`features.ingestion` on since 2026-07-12; invariant, cdk-nag, and snapshot suites; nothing is deployed yet: the account id in `infra/config/prod.ts` is a placeholder and the pipelines' deploy jobs stay dormant until the AWS role variables are set). Phase 2.5 (ASX import) is the next build.
